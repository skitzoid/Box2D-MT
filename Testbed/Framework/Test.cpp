...
void Test::Step(Settings* settings)
{
	...
	if (m_timeStep > 0.0f)
	{
		...
		m_maxProfile.solveTOIComputeTOI = b2Max(m_maxProfile.solveTOIComputeTOI, p.solveTOIComputeTOI);
	...
	if (settings->drawProfile && m_visible)
	{
		...
		if (m_stepCount > 0)
		{
			...
			aveProfile.solveTOIComputeTOI = scale * m_totalProfile.solveTOIComputeTOI;
		...
		m_textLine += DRAW_STRING_NEW_LINE;
		g_debugDraw.DrawString(5, m_textLine, "| |-compute TOIs [ave] (max) = %5.2f [%6.2f] (%6.2f)", p.solveTOIComputeTOI, aveProfile.solveTOIComputeTOI, m_maxProfile.solveTOIComputeTOI);
		m_textLine += DRAW_STRING_NEW_LINE;
